...
- Streams paradigm: want result of each stage as close as possible to _pure function_ of the result of the previous stage
...
- To avoid liveness and safety failures, never cede control to the client within a synchronized method or block, including _alien_ methods
...
- In a multicore world, the real cost of excessive synchronization is not the CPU time spent getting locks; it is _contention_: the lost opportunities for parallelism and the delays imposed by the need to ensure that every core has a consistent view of memory
